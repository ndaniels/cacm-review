...
\textbf{chemogenomics} Computational study of drugs and their targets based on chemical structure and function

\textbf{metagenomics} Study of the many genomes that make up a particular environment

\textbf{shotgun sequencing} Modern genomic sequencing, which chops DNA into many short pieces
...
\textbf{transcriptome} Transcribed RNA from a genome, which results in protein production

\textbf{BLAST} Standard biological sequence similarity search tool
...
Basic Local Alignment Search Tool (BLAST)~\cite{berger2013computational} has been
...
approaches, but slightly improves the accuracy of downstream 
variant-calling~\cite{yu2015quality}.
...
Similarly, for the sequence string,
taking advantage of sequence redundancy, Mince~\cite{patro2015data} groups 
similar reads (those that share a common short---15bp---substring) together into `buckets', allowing that common substring to now be 
